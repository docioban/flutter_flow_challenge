...
import 'package:doodle_dash/game/sprites/powerup.dart';
...
class Player extends SpriteGroupComponent<PlayerState>
  ...
  void update(double dt) {
    ...
    if (gameRef.gameManager.isGameOver || gameRef.gameManager.isIntro) return;
  ...
  void onCollision(Set<Vector2> intersectionPoints, PositionComponent other) {
    ...
    if (isMovingDown && isCollidingVertically) {
      ...
      if (other is NormalPlatform) {
        ...
        return;
          ...
          other.current == BrokenPlatformState) {
        jump();
        other.breakPlatform();
        return;
      }
    ...
    if (!hasPowerup && other is Rocket) {
      ...
      if (current == PlayerState.left) current = PlayerState.left;
      if (current == PlayerState.right) current == PlayerState.right;
      other.removeFromParent();
